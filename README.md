...
## Modern SaaS Upgrade (v2)

The project now includes a clean service + API architecture and a new SaaS dashboard UI.

### New app modules

- `accounts`: account-focused API endpoints
- `expenses`: expense API + serializers + services
- `income`: income API + serializers + services
- `analytics`: dashboard UI, analytics services, analytics APIs
- `goals`: goal API + serializers + services
- `reports`: report snapshot API
...
static/css/
  saas-dashboard.css
  theme-refresh.css
  theme-variant-zen.css
  theme-variant-carbon.css
static/js/
  saas-dashboard.js
  theme-toggle.js
```
...
- `GET/POST /income/`
- `GET/PUT/PATCH/DELETE /income/<id>/`
- `GET/POST /goals/`
- `GET/PUT/PATCH/DELETE /goals/<id>/`
- `GET /analytics/dashboard/`
- `GET /analytics/monthly-expense-summary/?months=6`
- `GET /analytics/income-vs-expense/?months=6`
- `GET /analytics/category-spending/?start_date=YYYY-MM-DD&end_date=YYYY-MM-DD`
- `GET /analytics/forecast-prediction/?days=30`
- `GET /reports/snapshot/?start_date=YYYY-MM-DD&end_date=YYYY-MM-DD`

### Dashboard page

- URL: `/dashboard/`
- Includes:
  - income/expense/savings/balance stat cards
  - income vs expense line chart
  - category pie chart
  - searchable/sortable/paginated recent transactions table
  - goals progress section
  - add/edit expense modal via API
  - loading overlay
  - dark mode toggle

### Filtering and pagination

Expense and income list APIs support:
- `q` (description search)
- `start_date`, `end_date`
- `min_amount`, `max_amount`
- `category` (expense), `source` (income)
- `ordering` (`date`, `-date`, `amount`, `-amount`, etc.)
- default DRF pagination (`page`, `count`, `next`, `previous`, `results`)

### Caching

Dashboard analytics endpoints use server-side cache (`LocMemCache`) for expensive aggregate queries.
